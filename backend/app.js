...
io.on('connection', (socket) => {
    ...
    socket.on('create-room', (data) => {
        ...
        console.log('create room event', data);
        socket.join(roomId);
    ...
    socket.on('join-room', (data) => {
        ...
        socket.join(roomId);
        emailToSocketMap.set(userEmail, socket.id);
        socket.broadcast.to(roomId).emit('user-connected', { email: userEmail, userId: myId })
        socket.emit('user-join', { roomId });
    });

    socket.on('call-user', (data) => {
        const { email, callerId, fromEmail, userId, offer } = data;
        console.log('call user event', data);
        socket.to(userId).emit('incoming-call', { fromEmail, callerId, offer })
    });

    socket.on('call-accepted', (data) => {
        const { userEmail,answer, callerId } = data;
        console.log('call Accepted event', data);
        socket.to(callerId).emit('call-accepted', {email: userEmail, answer, userId: socket.id });
